...
- **Never `kubectl apply` a dumped live object.** A live dump carries server-set fields (`spec.volumeName`, `volumeMode`, provisioner annotations) into `last-applied-configuration`. The repo manifests omit them, so the next CI apply computes "remove `volumeName`" and dies on `PersistentVolumeClaim … is invalid: spec: Forbidden: spec is immutable after creation` (broke run 31264534899, all 3 PVCs). Repair without touching spec: `kubectl apply set-last-applied -n nextcloud -f <repo-manifest>`, then confirm `kubectl diff -f <same>` is empty.
...
- No TLS terminates in this namespace. Shared Mesh :443 is `edge-proxy` (ns `ingress`), owned by the k3s-cluster repo, using its own `edge-mesh-tls`. The per-namespace nginx-tls-proxy was retired 2026-08-08 and its manifest deleted 2026-09-03, along with the `nextcloud-tls` Certificate and `nextcloud-https` Service it defined — all in git history.
